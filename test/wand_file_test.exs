defmodule WandFileTest do
  ...
  describe "save" do
    ...
    test "saves a dependency without a requirement" do
      file = %WandFile{
        dependencies: [
          %Dependency{
            name: "poison",
  ...
  defp stub_read(:ok, path, contents) do
    expect(WandCore.FileMock, :read, fn ^path -> {:ok, contents} end)
  end

  defp stub_read(:error, path, error) do
    expect(WandCore.FileMock, :read, fn ^path -> {:error, error} end)
  end
